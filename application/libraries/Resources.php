...
class Resources {
	...
	function js() {
		...
		foreach ($this->js as $key => $val) {
			...
			// Si es entero es un arreglo unidimensional
			if(!is_int($key)) {
				$content .= '<script type="text/javascript" defer src="' . base_url() . $val . $key . '.js"></script>';
			}
			else {
				// Se verifica si el valor enviado es una URL correcta
				if(filter_var($val, FILTER_VALIDATE_URL)) {
					$content .= '<script type="text/javascript" defer src="' . $val . '"></script>';
				}
				else {
					$content .= '<script type="text/javascript" defer src="' . base_url() . $this->js_path . $val . '.js"></script>';
				}
			}
		}
		return $content;
	...
	function functions() {
		if (!empty($this->functions)) {
			$content = '<script type="text/javascript" defer >$(function() {';
