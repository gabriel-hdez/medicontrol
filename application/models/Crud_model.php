...
//--------------------------------------------------------------------- LEER O SELECCIONAR
    public function read($data)
    {
        ...
        {  
            ...
            {
                if ($this->db->field_exists('estado', $key) ) 
                {
                    if ($this->db->field_exists('estado', $data['table'])) 
                    {
                        $as .= ' , '.$data['table'].'.estado AS estado ,'.$data['table'].'.fecha_modificacion AS fecha_modificacion ,'.$key.'.estado AS estado'.$i.' ,'.$key.'.fecha_modificacion AS fecha_modificacion'.$i;
                        $i++;
                    } 
                    else 
                    {
                        $as .= ' , '.$key.'.estado AS estado'.$i.' ,'.$key.'.fecha_modificacion AS fecha_modificacion'.$i;
                        $i++;
                    }
        ...
        if (isset($data['join'])) 
        {
            foreach ($data['join'] as $key => $value) 
            {
                $params = substr(strchr($value , ','),2);
                // params = LEFT, RIGHT, INNER, OUTER, OUTER LEFT, OUTER RIGHT
                if ($params !== FALSE) 
                {
                    $value2 = str_replace( strchr($value , ',') , '' , $value);
                    $this->db->join($key , $value2 , $params);
                }
                else
                {
...
//--------------------------------------------------------------------- AUTENTICACION DE USUARIOS
    public function login($data)
    {
        ...
        {            
            ...
            //$this->db->join('perfiles', 'perfiles.id_perfil = usuarios.id_perfil');
            //$this->db->join('privilegios', 'privilegios.id_perfil = usuarios.id_perfil');
