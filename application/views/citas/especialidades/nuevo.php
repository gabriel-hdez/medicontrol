...
?>
    ...
    <form data-response="json" method="POST" action="<?php echo base_url('citas/especialidades/guardar');?>">
        <div class="row">
            <div class="col s12 l3 input-field">
                <select name="id_especialidad" id="id_especialidad" class="select2 browser-default select-especialidad" >
                    ...
                    <?php foreach($especialidades as $item): ?>
                        <option value="<?php echo $item->id_especialidad; ?>" ><?php echo $item->especialidad; ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="col s12 l4 input-field">
                <select name="id_personal" id="id_personal" class="select2 browser-default select-personal" >
                    ...
                    <?php foreach($personal as $item): ?>
                        <option value="<?php echo $item->id_personal; ?>" ><?php echo substr($item->tipo_persona, 0,1).'-'.$item->cedula.', '.$item->nombres.' '.$item->apellidos; ?></option>
                    <?php endforeach; ?>
                </select>
                    ...
                    <tbody>
                        <?php for ($i=1; $i<= 7; $i++): ?>
                            <tr>
                                ...
                                <?php foreach($dias as $dia): if($dia->id_dia == $i): ?>
                                    <td class="left"><?php echo $dia->dia;?></td>
                                <?php endif; endforeach; ?>
                                
                                <td class="center">
                                    <input type="text" name="hora_inicio[]" id="<?php echo 'inicio_'.$i;?>" class="timepicker" placeholder="" disabled>
                                </td>
                                
                                <td class="center">
                                    <input type="text" name="hora_final[]" id="<?php echo 'final_'.$i;?>" class="timepicker" placeholder="" disabled>
                                </td>
                            </tr>
                        <?php endfor; ?>
                    </tbody>
